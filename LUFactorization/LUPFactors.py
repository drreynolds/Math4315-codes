...
def LUPFactors(A):
    ...
    m, n = numpy.shape(A)
    if (m != n):
        raise ValueError("LUPFactors error: matrix must be square")
    ...
    for k in range(n-1):             # loop over pivots
      ...
      for i in range(k+1,n):
        if (numpy.abs(U[i,k]) > numpy.abs(U[s,k])):
          s = i
      if (numpy.abs(U[s,k]) < tol):  # check for singularity
        raise ValueError("LUPFactors error: matrix is [close to] singular")
